        ...
        <div className="grid grid-cols-3 md:grid-cols-2 lg:grid-cols-3 gap-x-3 gap-y-6 md:gap-6 mb-8 md:mb-12">
          ...
          {displayProducts.map((prod, idx) => (
            <Link href={`/products/${prod.slug}`} key={idx} className="group block">
              ...
              <h3 className="text-gray-900 font-medium md:font-semibold text-[11.5px] md:text-[1.1rem] px-0.5 md:px-1 leading-tight group-hover:text-meewa-red transition-colors">{prod.name}</h3>
            </Link>
          ))}

        </div>

        {/* CTA Card - Full Width Below Products */}
        <div className="w-full">
          <div className="w-full rounded-xl bg-[#FDF2F3] p-6 md:p-10 flex flex-col justify-center border border-red-100 shadow-sm">
            <h3 className="text-meewa-red text-[20px] md:text-3xl font-medium mb-2 md:mb-4 leading-tight">Need something specific?</h3>
            <p className="text-gray-800 text-[11.5px] md:text-base mb-6 md:mb-8 max-w-3xl leading-relaxed">
              Share your sizes, artwork, and volumes — we quote within 48 hours with export-ready pricing.
            </p>
            <div>
              <Link href="/contact" className="bg-meewa-red text-white px-5 py-2.5 md:px-6 md:py-3 rounded-full text-[11px] md:text-sm font-medium hover:bg-red-700 transition-colors inline-flex items-center">
                Request a Quote
                <svg className="w-3.5 h-3.5 md:w-4 md:h-4 ml-1.5 md:ml-2 transform -rotate-45" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
              </Link>
            </div>
          </div>
        </div>
